...
################################################################################
##### MAIN ANALYSIS (PER-CELL TYPE) ############################################

{
  # Initial definition of data structures for saving per-cell type results
  MEs_LIST = list()
  NETs_LIST = list()
  DO_GSEA = T              # Perform/Not perform GSEA analysis (for gene sets corresponding to the genes associated with each module)
  GSEA = list()            # for saving per-cell-type GSEA results
  DAC_ENRICH = list()      # for saving enrichment of differential (SCZ case-control) OCRs in OCRs sets associated with each module (generating per-module statistics)
  ...
  # Vocabulary for alternative cell type names
  ctypeList = list(
    "GABA" = list(short="GABA", long="GABAergic"),
    "GLU" = list(short="GLU", long="glutamatergic"),
    "OLIG" = list(short="OLIG", long="oligodendrocytes"),
    "MGAS" = list(short="MGAS", long="microgliaAndAstrocytes")
  )
  
  # Let's run gene coexpression analysis separately for each cell type
  for(CTYPE in CELL_TYPES) {
    qcRnaSub = qcRna[qcRna$cell_subtype_abbreviation == CTYPE,]
    ovrl_subjects = intersect(qcAtac[qcAtac$cell_subtype_abbreviation == CTYPE, "Subject_ID"], qcRnaSub[qcRnaSub$cell_subtype_abbreviation == CTYPE, "Subject_ID"])
    ids_atacseq = qcAtac[(qcAtac$cell_subtype == CTYPE),]
    ids_atacseq = ids_atacseq[match(ovrl_subjects, ids_atacseq$Subject_ID), "ID"]
    ids_rnaseq = qcRnaSub[match(ovrl_subjects, qcRnaSub$Subject_ID), "ID"]
    
    CTYPE_ALT = ctypeList[[CTYPE]]$long
    
    ###
    ### Network construction and module detection (incl. dendrogram plotting)
    {
      expr = t(rnaseq_countMatrixResiDxCellSep[[CTYPE]][,ids_rnaseq])
      chosen_power = 7
    ...
    ### Eigengenes and disease association
    {
      ...
      mpdf(paste0("misc_eigen_dx_correl_", CTYPE), outDir=file.path(ROOT, "outputs")); print(eigenPlot); dev.off()
    }
    ...
    # Permutation tests to calculate whether we have significantly more modules (in each cell type) that have more than one OCRs significantly associated with module eigenvector
    {
      ...
      mpdf(paste0("misc_wgcna_permutPlot_", CTYPE), outDir=file.path(ROOT, "outputs"), width=4, height=2); print(permutPlot); dev.off()
    }
    ...
    write.csv(enrichTable, file=file.path(ROOT, "outputs", paste0("misc_WGCNA_dac_", CTYPE, ".csv")), row.names=F)
    ...
    write.csv(magmaScz, file=file.path(ROOT, "outputs", paste0("misc_WGCNA_magma_", CTYPE, ".csv")), row.names=F)
  }
  ...
  # Plotting
  {
    ...
    wgcnaPlot
    mpdf("misc_WGCNA_modules_by_ctype", outDir=file.path(ROOT, "outputs"), width=4, height=2); print(wgcnaPlot); dev.off()
  }
  
  # Summary numbers for this analysis
  {
  ...
  # Plot correlation between modules
  {
    for(ctype in CELL_TYPES) {
      ...
      mpdf(paste0("wgcna_modules_", ctype), outDir=file.path(ROOT, "outputs"), width=4, height=4) 
      heatmap(
        MEcor,
      ...
      mpdf(file.path(paste0("misc_wgcna_eigen_network_", ctype)), outDir=file.path(ROOT, "outputs"), width=8, height=7)
  ...
  # Plot intramodular coherence across cell types
  {
    hub_thresh = 0.6
    
    for(ctype in CELL_TYPES) {
      kME = cor(t(rnaseq_countMatrixResiDxCellSep[[ctype]][,rownames(MEs_LIST[[ctype]])]), MEs_LIST[[ctype]], use = "pairwise.complete.obs")
      kME_df = data.frame(
        gene = rownames(rnaseq_countMatrixResiDxCellSep[[ctype]][,rownames(MEs_LIST[[ctype]])]),
      ...
      kME_df$kME = mapply(function(g, m) {
        if(m == "grey") return(NA)
        kME[g, paste0("ME", m)]
      }, g = rownames(rnaseq_countMatrixResiDxCellSep[[ctype]][,rownames(MEs_LIST[[ctype]])]), m = labels2colors(NETs_LIST[[ctype]]$colors))
      ...
      mpdf(paste0("wgcna_module_kME_full_", ctype), outDir=file.path(ROOT, "outputs"), width=8, height=6); print(kmePlot); dev.off()
      ...
      mpdf(paste0("misc_wgcna_module_kME_threshold_", ctype), outDir=file.path(ROOT, "outputs"), width=2, height=3); print(hubSumPlot); dev.off()
    }
  }
  
  ###
  # Plot fFraction of modules with ≥1 regulatory OCR
  {
    df = eigenVectorCorrelSum
    ...
    mpdf("misc_wgcna_addAtac_propNonZero", outDir=file.path(ROOT, "outputs"), width=3, height=3); print(propNonZeroPlot); dev.off()
    ...
    mpdf("misc_wgcna_addAtac_signOcrPerModPlot", outDir=file.path(ROOT, "outputs"), width=5, height=3); print(signOcrPerModPlot); dev.off()
    
    mpdf("misc_wgcna_minorRank", outDir=file.path(ROOT, "outputs"), width=5, height=3)
    ...
    mpdf("misc_wgcna_addAtac_strength_3", outDir=file.path(ROOT, "outputs"), width=5, height=3); print(strengthPlot3); dev.off()
  }
}
